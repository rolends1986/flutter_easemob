...
- (void)setupCell {
    ...
    [[HDBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:HDUIResourceItemPath(@"chat_sender_bg")] stretchableImageWithLeftCapWidth:5 topCapHeight:35]];
    [[HDBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:HDUIResourceItemPath(@"chat_receiver_bg")] stretchableImageWithLeftCapWidth:35 topCapHeight:35]];
    [[HDBaseMessageCell appearance] setSendMessageVoiceAnimationImages:@[[UIImage imageNamed:HDUIResourceItemPath(@"chat_sender_audio_playing_full")], [UIImage imageNamed:HDUIResourceItemPath(@"chat_sender_audio_playing_000")], [UIImage imageNamed:HDUIResourceItemPath(@"chat_sender_audio_playing_001")], [UIImage imageNamed:HDUIResourceItemPath(@"chat_sender_audio_playing_002")], [UIImage imageNamed:HDUIResourceItemPath(@"chat_sender_audio_playing_003")]]];
    [[HDBaseMessageCell appearance] setRecvMessageVoiceAnimationImages:@[[UIImage imageNamed:HDUIResourceItemPath(@"chat_receiver_audio_playing_full")],[UIImage imageNamed:HDUIResourceItemPath(@"chat_receiver_audio_playing000")], [UIImage imageNamed:HDUIResourceItemPath(@"chat_receiver_audio_playing001")], [UIImage imageNamed:HDUIResourceItemPath(@"chat_receiver_audio_playing002")], [UIImage imageNamed:HDUIResourceItemPath(@"chat_receiver_audio_playing003")]]];
...
- (void)setLeftBarBtnItem {
    ...
    [backButton setImage:[UIImage imageNamed: [NSString stringWithFormat:@"%@/Shape",HDResourcePackage]] forState:UIControlStateNormal];
...
#pragma mark smallpngface
        ...
        for (int i=1; i<=35; i++) {
            // 把自定义表情图片加到数组中
            customName = [HDUIResourceItemPath(@"e_e_") stringByAppendingString:[NSString stringWithFormat:@"%d",i]];
...
- (void)didHDStartRecordingVoiceAction:(UIView *)micView
{
    ...
    [self _canRecordCompletion:^(HDRecordResponse recordResponse) {
        switch (recordResponse) {
            ...
            case HDCanNotRecord:
            {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:HDLocalizedString(@"prompt", @"Prompt") message:HDLocalizedString(@"message.failToPermission", @"No recording permission") delegate:nil cancelButtonTitle:HDLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
                [alertView show];
            }
                break;
            default:
...
-(void)visitorWaitCount:(int)count{
    if (count > 0) {
        if (_visitorWaitCountLabel) {
            _visitorWaitCountLabel.text = [NSString stringWithFormat:HDLocalizedString(@"current_visitor_wait_count", @" The current queue number is ：%d"), count];
...
- (void)commitSatisfactionWithControlArguments:(ControlArguments *)arguments type:(ControlType *)type evaluationTagsArray:(NSMutableArray *)tags{
    ...
    __weak typeof(self) weakself = self;
    [self showHudInView:self.view hint:HDLocalizedString(@"comment_submit", @"Comment Submit.")];
    [[HDClient sharedClient].chatManager sendMessage:message progress:nil completion:^(HDMessage *aMessage, HDError *aError) {
        [self hideHud];
        if (!aError) {
            [weakself.tableView reloadData];
            [weakself showHint:HDLocalizedString(@"comment_suc", @"Add comment successful.")];
        } else {
            [weakself showHint:HDLocalizedString(@"comment_fail", @"Add comment fail.")];
        }
