...
- (UIImageView*)headImage
{
    if (_headImage == nil) {
        ...
        _headImage.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/customer",HDResourcePackage]];
    }
    return _headImage;
}
...
- (UILabel*)nickLabel
{
    if (_nickLabel == nil) {
        ...
        _nickLabel.text = HDLocalizedString(@"satisfaction.nickname", @"Nickname");
...
- (UILabel*)textLabel
{
    if (_textLabel == nil) {
        ...
        _textLabel.text = HDLocalizedString(@"satisfaction.message", @"please evaluate my service");
...
- (UIButton*)commitBtn
{
    if (_commitBtn == nil) {
        ...
        [_commitBtn setTitle:HDLocalizedString(@"satisfaction.commit", @"commit") forState:UIControlStateNormal];
...
- (void)commit
{
    if (!_starRateView.isTap && _starRateView.scorePercent != 1) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:HDLocalizedString(@"satisfaction.alert", @"please evaluate first") delegate:self cancelButtonTitle:HDLocalizedString(@"cancela", @"Cancel") otherButtonTitles:HDLocalizedString(@"ok", @"Ok"), nil];
        [alert show];
        return;
    }
        ...
        if ([self.messageModel.message.ext objectForKey:kMesssageExtWeChat]) {
            ...
            if ([weichat objectForKey:kMesssageExtWeChat_ctrlArgs]) {
                ...
                if (self.evaluationTagsArray.count == 0 && self.evaluationTagView.evaluationDegreeModel.appraiseTags.count>0) {
                    [self showHint:HDLocalizedString(@"select_at_least_one_tag", @"Select at least one tag!")];
                } else {
