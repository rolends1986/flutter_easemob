...
- (instancetype)initWithFrame:(CGRect)frame tag:(NSInteger)tag
{
    ...
    if (self) {
        NSString *path = [HDUIResourceBundle pathForResource:@"commodityInfo.plist" ofType:nil];
...
- (void)setUIWidth:(CGFloat)width height:(CGFloat)height tagNum:(NSInteger)tag
{
    ...
    self.name.text = HDLocalizedString([_comDataSource[tag] objectForKey:@"name"], @"em_example1_text");
    ...
    self.novemberSales.text = HDLocalizedString([_comDataSource[tag] objectForKey:@"novemberSales"], @"em_example1_text_description");
