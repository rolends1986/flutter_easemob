...
- (void)setupBarButtonItem
{
    ...
    [backButton setTitle:HDLocalizedString(@"leave_title", @"Note") forState:UIControlStateNormal];
...
//    });
    ...
    [button setImage:[UIImage imageNamed:HDUIResourceItemPath(@"hd_icon_leave_suc")] forState:UIControlStateNormal];
    [self.view addSubview:button];
    
    UILabel *commitLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(button.frame) + 20, kScreenWidth, 30)];
    commitLabel.text = HDLocalizedString(@"new_leave_send_success", @"Submit successful");
    ...
    thankLabel.text = HDLocalizedString(@"new_leave_send_descriptionOne", @"Thank you for your leave message.");
    ...
    thankLabelOther.text = HDLocalizedString(@"new_leave_send_descriptionTwo", @"We will give you a response at the first time.");
    ...
    NSArray *placeholders = @[HDLocalizedString(@"ticket_name", @"Name"),HDLocalizedString(@"ticket_phone", @"Phone"),HDLocalizedString(@"ticket_email", @"Email"),HDLocalizedString(@"ticket_theme", @"Theme"),HDLocalizedString(@"ticket_detail", @"Detail")];
    for (int i=0; i<5; i++) {
