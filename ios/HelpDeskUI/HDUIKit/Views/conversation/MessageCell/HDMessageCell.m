                         ...
                         model:(id<HDIMessageModel>)model
{
    ...
    [_statusButton setImage:[UIImage imageNamed:HDUIResourceItemPath(@"messageSendFail")] forState:UIControlStateNormal];
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (messageType) {
            ...
            case EMMessageBodyTypeImage:
            {
                ...
                _bubbleView.imageView.image = [UIImage imageNamed:HDUIResourceItemPath(@"imageDownloadFail")];
            }
                break;
            case EMMessageBodyTypeLocation:
            {
                ...
                _bubbleView.locationImageView.image = [[UIImage imageNamed:HDUIResourceItemPath(@"chat_location_preview")] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
...
- (void)setModel:(id<HDIMessageModel>)model
{
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
            {
                ...
                switch (extMsgType) {
                    case HDExtOrderMsg:
                    {
                        ...
                        [_bubbleView.orderImageView hdSD_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:HDUIResourceItemPath(@"imageDownloadFail")]];
                    ...
                    case HDExtTrackMsg:
                    {
                        ...
                        if ([imageName length] > 0) {
                            ...
                            _bubbleView.cusImageView.image = [UIImage imageNamed:HDUIResourceItemPath(@"imageDownloadFail")];
                        }
                        NSString *url = [itemDic objectForKey:@"img_url"];
                        [_bubbleView.cusImageView hdSD_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:HDUIResourceItemPath(@"imageDownloadFail")]];
                    ...
                    case HDExtEvaluationMsg:
                    {
                        _bubbleView.evaluateTitle.attributedText = [[NSAttributedString alloc] initWithString:HDLocalizedString(@"satisfaction.message", @"please evaluate my service")];
                    }
                        break;
                    case HDExtToCustomServiceMsg: {
                    ...
                    case HDExtFormMsg:
                    {
                        ...
                        _bubbleView.formIconView.image = [UIImage imageNamed:HDUIResourceItemPath(@"chat_item_form")];
                        @try {
            ...
            case EMMessageBodyTypeVoice:
            {
                if (_bubbleView.voiceImageView) {
                    if ([self.sendMessageVoiceAnimationImages count] > 0 && [self.recvMessageVoiceAnimationImages count] > 0) {
                        ...
                        self.bubbleView.voiceImageView.image = self.model.isSender ?[UIImage imageNamed:HDUIResourceItemPath(@"chat_sender_audio_playing_full")]: [UIImage imageNamed:HDUIResourceItemPath(@"chat_receiver_audio_playing_full")];
                    }
                }
                if (!self.model.isSender) {
            ...
            case EMMessageBodyTypeVideo:
            {
                _bubbleView.fileIconView.image = [UIImage imageNamed:HDUIResourceItemPath(_model.fileIconName)];
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText: {
            ...
            switch (extMsgType) {
                ...
                case HDExtEvaluationMsg:
                {
                    NSAttributedString *text = [[NSAttributedString alloc] initWithString: HDLocalizedString(@"satisfaction.message", @"please evaluate my service")];
                    CGRect rect = [text boundingRectWithSize:CGSizeMake(bubbleMaxWidth - 25, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading  context:nil];
                    height += (rect.size.height > 20 ? rect.size.height : 20) + 21;
                    height += 50;
                    return height;
